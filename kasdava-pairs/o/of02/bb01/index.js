// Next SDK Integration - Bundle Page

// Package ID Calculator for Kasdava Master Products
// Note: 4-pair bundles are handled as 3-pair bundle + 1 individual pair
// This means the first 3 pairs use quantity=3 package IDs, and the 4th pair uses quantity=1 package IDs
function calculatePackageId(color, size, quantity) {
    ...
    const colorConfig = {
        'brown':      { start: 1,  sizes: 14, hasExtended: true },  // 1-14
        'blue':       { start: 15, sizes: 14, hasExtended: true },  // 15-28
        'green':      { start: 29, sizes: 14, hasExtended: true },  // 29-42
        'black':      { start: 43, sizes: 14, hasExtended: true },  // 43-56
        'dark-blue':  { start: 57, sizes: 14, hasExtended: true },  // 57-70
        'gray':       { start: 71, sizes: 14, hasExtended: true },  // 71-84
        'pink':       { start: 85, sizes: 9,  hasExtended: false }  // 85-93 (9 sizes)
    };
    
    // All sizes for Kasdava Master (14 sizes total)
    const allSizes = [
        'US Women 5 - US Men 3',
        'US Women 6 - US Men 4',
        'US Women 7 - US Men 5',
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const redEl = document.querySelector(".pf-kasdava-buy-shop-notice-title.red");
    const greenEl = document.querySelector(".pf-kasdava-buy-shop-notice-title.green");
...
document.addEventListener('DOMContentLoaded', function() {
    const imagesList = document.querySelector('.pf-kasdava-buy-images-list');
    const leftNav = document.querySelector('.pf-kasdava-buy-images-nav-left');
    const rightNav = document.querySelector('.pf-kasdava-buy-images-nav-right');
    const thumbnails = document.querySelectorAll('.pf-kasdava-buy-images-thumbnails-item');
...
document.addEventListener('DOMContentLoaded', function() {
    const backButton = document.querySelector('.pf-kasdava-back-btn');
...
document.addEventListener('DOMContentLoaded', function() {
    function checkMobileLayout() {
        const stickyATC = document.querySelector('.pf-kasdava-buy-shop-atc');
...
document.addEventListener('DOMContentLoaded', function() {
    const sizeGuideButton = document.querySelector('.pf-kasdava-buy-shop-sg');
    const sizeGuideModal = document.getElementById('pf-kasdava-size-guide-modal');
    ...
    function openModal() {
        ...
        const closeButton = sizeGuideModal.querySelector('.pf-kasdava-size-guide-modal-close');
        if (closeButton) {
            closeButton.focus();
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateProductImage(container, selectedColor) {
        const productImage = container.querySelector('.pf-kasdava-buy-shop-item-conf-product-image');
    ...
    selects.forEach(select => {
        select.addEventListener('change', function() {
            const container = this.closest('.pf-kasdava-buy-shop-item-container');
            ...
            // Log selection changes and calculate package ID
            if (colorSelect && sizeSelect && colorSelect.value && sizeSelect.value) {
                const activeContainers = document.querySelectorAll('.pf-kasdava-buy-shop-item-container.active');
                const currentQuantity = activeContainers.length;
                const containerIndex = Array.from(activeContainers).indexOf(container);
                
                // Handle 4-pair bundle as 3-pair bundle + 1 individual pair
                let quantityForPackageId = currentQuantity;
                if (currentQuantity === 4) {
                    if (containerIndex === 3) {
                        quantityForPackageId = 1; // 4th pair uses individual pricing
                    } else {
                        quantityForPackageId = 3; // First 3 pairs use 3-pair bundle pricing
                    }
                }
                
                // Debug the calculation
                console.log('Calculating package ID for:', {
                    color: colorSelect.value,
                    size: sizeSelect.value,
                    quantity: quantityForPackageId,
                    containerIndex: containerIndex,
                    totalItems: currentQuantity
                });
                
                const packageId = calculatePackageId(colorSelect.value, sizeSelect.value, quantityForPackageId);
                
                console.log(`Pair ${container.dataset.item} configured: ${colorSelect.value} - ${sizeSelect.value} (Package ID: ${packageId})`);
                
                if (!packageId) {
                    console.error('Package ID calculation failed for:', colorSelect.value, sizeSelect.value, quantityForPackageId);
                }
            }
        ...
        if (select.name === 'color') {
            const container = select.closest('.pf-kasdava-buy-shop-item-container');
            const sizeSelect = container.querySelector('select[name="size"]');
            if (sizeSelect) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const galleryNavButtons = document.querySelectorAll('.pf-kasdava-buy-images-nav-left, .pf-kasdava-buy-images-nav-right');
    galleryNavButtons.forEach(button => {
        button.setAttribute('role', 'button');
        button.setAttribute('tabindex', '0');
        
        if (button.classList.contains('pf-kasdava-buy-images-nav-left')) {
...
function fireAddToCartEvent(bundleConfig, totalValue) {
    ...
    const trackingItems = bundleConfig.map((item, index) => ({
        item_id: `kasdava-master-${item.packageId}`,
        item_name: `Kasdava Master - ${item.color} - ${item.size}`,
        ...
        quantity: 1,
        item_sku: `KASDAVA-${item.packageId}`,
        item_image: 'https://cdn.29next.store/media/peakfootwear/uploads/brown.png'
    }));
    ...
    if (window.UnifiedTrackingBridge && window.UnifiedTrackingBridge.track) {
        const cartData = {
            cartLines: bundleConfig.map(item => ({
                ...
                product: {
                    sku: `KASDAVA-${item.packageId}`,
                    title: `Kasdava Master - ${item.color} - ${item.size}`,
...
window.addEventListener('next:initialized', function() {
    const checkoutButton = document.querySelector('.pf-kasdava-buy-shop-atc-button');
    ...
    if (checkoutButton) {
        checkoutButton.addEventListener('click', async function(e) {
            ...
            // Handle 4-pair bundle as 3-pair bundle + 1 individual pair
            let bundleQuantityForPackageId = currentQuantity;
            if (currentQuantity === 4) {
                bundleQuantityForPackageId = 3; // Use 3-pair bundle pricing for all items
            }
            ...
            for (const [index, container] of activeContainers.entries()) {
                ...
                if (colorSelect && sizeSelect && colorSelect.value && sizeSelect.value) {
                    ...
                    // For 4th pair, use individual pair pricing (quantity=1)
                    let packageIdQuantity = bundleQuantityForPackageId;
                    if (currentQuantity === 4 && index === 3) {
                        packageIdQuantity = 1; // 4th pair uses individual pricing
                    }
                    
                    const packageId = calculatePackageId(color, size, packageIdQuantity);
                    
                    if (packageId) {
                        console.log(`Adding to cart: ${color} ${size} (Package ID: ${packageId}, Quantity Base: ${packageIdQuantity})`);
                        ...
                        try {
                            ...
                            bundleConfig.push({
                                ...
                                size: size,
                                packageId: packageId,
                                quantityBase: packageIdQuantity
                            });
                        } catch (error) {
            ...
            setTimeout(() => {
                ...
                const checkoutUrl = preserveUrlParameters('/kasdava-pairs/c/co01');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const selectedQuantity = sessionStorage.getItem('pfKasdavaSelectedQuantity');
    const quantity = selectedQuantity ? parseInt(selectedQuantity) : 1;
    
    // Update the pair count display
    const pairCountEl = document.querySelector('.pf-kasdava-buy-shop-tb-pc');
    if (pairCountEl) {
        pairCountEl.textContent = quantity;
    }
    
    // Show the correct number of active items based on selection
    const shopItems = document.querySelectorAll('.pf-kasdava-buy-shop-item-container');
    ...
    const pricingData = {
        1: { total: 69.95, originalTotal: 145.95, savings: 52 },
        2: { total: 125.90, originalTotal: 291.90, savings: 57 },
        3: { total: 167.85, originalTotal: 437.85, savings: 62 },
        4: { total: 223.80, originalTotal: 583.80, savings: 62 } // 4 pairs at $55.95 each
    };
    
    const pricing = pricingData[quantity];
    if (pricing) {
        const priceMainEl = document.querySelector('.pf-kasdava-buy-shop-atc-price-main');
        const priceSuEl = document.querySelector('.pf-kasdava-buy-shop-atc-price-su');
        const savingsEl = document.querySelector('.pf-kasdava-buy-shop-atc-price-savings');
    ...
    const bundleConfig = [];
    const activeItems = document.querySelectorAll('.pf-kasdava-buy-shop-item-container.active');
...
document.addEventListener('DOMContentLoaded', function() {
    const addButtons = document.querySelectorAll('.pf-kasdava-buy-shop-item-ny-add-button');
    ...
    function updateBundleState() {
        ...
        if (pricing) {
            ...
            const priceSuEl = document.querySelector('.pf-kasdava-buy-shop-atc-price-su');
            const savingsEl = document.querySelector('.pf-kasdava-buy-shop-atc-price-savings');
            const priceContainer = document.querySelector('.pf-kasdava-buy-shop-atc-price');
            const checkoutSection = document.querySelector('.pf-kasdava-buy-shop-atc');
        ...
        sessionStorage.setItem('pfKasdavaSelectedQuantity', currentQuantity.toString());
        
        // Update step counter text if needed
        const stepTitle = document.querySelector('.pf-kasdava-buy-shop-tb-text');
        if (stepTitle) {
            const pairText = currentQuantity === 1 ? 'pair' : 'pairs';
            stepTitle.innerHTML = `Select <span class="pf-kasdava-buy-shop-tb-pc">${currentQuantity}</span> ${pairText}`;
        }
    ...
    function toggleBundleItem(container, activate) {
        if (activate) {
            ...
            if (colorSelect && sizeSelect) {
                // Kasdava: Update size options based on default color (Pink doesn't have extended sizes)
                const extendedSizeColors = ['brown', 'blue', 'green', 'black', 'dark-blue', 'gray'];
                const selectedColor = colorSelect.value;
                const extendedSizeOptions = sizeSelect.querySelectorAll('.extended-size');
                ...
                // Update product image (Kasdava Master)
                const colorImageMap = {
                    'brown': 'https://cdn.29next.store/media/peakfootwear/uploads/brown.png',
                    'blue': 'https://cdn.29next.store/media/peakfootwear/uploads/blue.jpg',
                    'green': 'https://cdn.29next.store/media/peakfootwear/uploads/green.jpg',
    ...
    addButtons.forEach((button, index) => {
        button.addEventListener('click', function(e) {
            ...
            const currentActiveCount = document.querySelectorAll('.pf-kasdava-buy-shop-item-container.active').length;
    ...
    function addRemoveButtons() {
        const configuredItems = document.querySelectorAll('.pf-kasdava-buy-shop-item-conf');
        
        configuredItems.forEach(item => {
            // Check if remove button already exists
            if (item.querySelector('.pf-kasdava-remove-button')) return;
            
            const removeButton = document.createElement('button');
            removeButton.className = 'pf-kasdava-remove-button';
            ...
            removeButton.addEventListener('click', function(e) {
                ...
                if (container) {
                    ...
                    const activeCount = document.querySelectorAll('.pf-kasdava-buy-shop-item-container.active').length;
                    if (activeCount <= 1) {
