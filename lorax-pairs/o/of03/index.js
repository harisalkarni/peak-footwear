...
// Countdown Timer functionality for pf-lorax sale timer
document.addEventListener("DOMContentLoaded", function () {
    ...
    console.log('Lorax countdown timer initialized successfully');
});

// Product Carousel Functionality
document.addEventListener("DOMContentLoaded", function () {
    // Product images for carousel
    const pfLoraxSliImages = [
    ...
    let pfLoraxSliCurrentIndex = 0;
    const pfLoraxSliCarouselTrack = document.getElementById('pf-lorax-sli-carouselTrack');
    const pfLoraxSliThumbnailsContainer = document.getElementById('pf-lorax-sli-thumbnailsContainer');
    const pfLoraxSliPrevBtn = document.getElementById('pf-lorax-sli-prevBtn');
    const pfLoraxSliNextBtn = document.getElementById('pf-lorax-sli-nextBtn');

    // Check if carousel elements exist
    if (!pfLoraxSliCarouselTrack || !pfLoraxSliThumbnailsContainer || !pfLoraxSliPrevBtn || !pfLoraxSliNextBtn) {
        console.log('Carousel elements not found, skipping carousel initialization');
        return;
    }

    // Initialize carousel with optimized loading
    function pfLoraxSliInitCarousel() {
        // Create slides
        pfLoraxSliImages.forEach((imgSrc, index) => {
            const slide = document.createElement('div');
            slide.className = 'pf-lorax-sli-carousel-slide';
            ...
            pfLoraxSliCarouselTrack.appendChild(slide);
        });

        // Create thumbnails with progressive loading
        pfLoraxSliImages.forEach((imgSrc, index) => {
            const thumbnail = document.createElement('div');
            thumbnail.className = 'pf-lorax-sli-thumbnail';
            if (index === 0) thumbnail.classList.add('pf-lorax-sli-active');
            ...
            thumbnail.addEventListener('click', () => pfLoraxSliGoToSlide(index));
            pfLoraxSliThumbnailsContainer.appendChild(thumbnail);
        });
    }

    // Update carousel position
    function pfLoraxSliUpdateCarousel() {
        pfLoraxSliCarouselTrack.style.transform = `translateX(-${pfLoraxSliCurrentIndex * 100}%)`;
        
        // Update active thumbnail
        document.querySelectorAll('.pf-lorax-sli-thumbnail').forEach((thumb, index) => {
            thumb.classList.toggle('pf-lorax-sli-active', index === pfLoraxSliCurrentIndex);
        });

        // Scroll thumbnail into view
        const activeThumbnail = document.querySelector('.pf-lorax-sli-thumbnail.pf-lorax-sli-active');
        if (activeThumbnail) {
    ...
    function pfLoraxSliGoToSlide(index) {
        pfLoraxSliCurrentIndex = index;
        pfLoraxSliUpdateCarousel();
    }

    // Next slide
    function pfLoraxSliNextSlide() {
        pfLoraxSliCurrentIndex = (pfLoraxSliCurrentIndex + 1) % pfLoraxSliImages.length;
        pfLoraxSliUpdateCarousel();
    }

    // Previous slide
    function pfLoraxSliPrevSlide() {
        pfLoraxSliCurrentIndex = (pfLoraxSliCurrentIndex - 1 + pfLoraxSliImages.length) % pfLoraxSliImages.length;
        pfLoraxSliUpdateCarousel();
    }

    // Event listeners for carousel
    pfLoraxSliPrevBtn.addEventListener('click', pfLoraxSliPrevSlide);
    pfLoraxSliNextBtn.addEventListener('click', pfLoraxSliNextSlide);

    // Keyboard navigation
    document.addEventListener('keydown', (e) => {
        if (e.key === 'ArrowLeft') pfLoraxSliPrevSlide();
        if (e.key === 'ArrowRight') pfLoraxSliNextSlide();
    });

    // Touch/swipe support
    let pfLoraxSliTouchStartX = 0;
    let pfLoraxSliTouchEndX = 0;

    pfLoraxSliCarouselTrack.addEventListener('touchstart', (e) => {
        pfLoraxSliTouchStartX = e.changedTouches[0].screenX;
    });

    pfLoraxSliCarouselTrack.addEventListener('touchend', (e) => {
        pfLoraxSliTouchEndX = e.changedTouches[0].screenX;
        pfLoraxSliHandleSwipe();
    });

    function pfLoraxSliHandleSwipe() {
        if (pfLoraxSliTouchEndX < pfLoraxSliTouchStartX - 50) pfLoraxSliNextSlide();
        if (pfLoraxSliTouchEndX > pfLoraxSliTouchStartX + 50) pfLoraxSliPrevSlide();
    }

    // Initialize carousel
    pfLoraxSliInitCarousel();
...
document.addEventListener("DOMContentLoaded", function () {
    const countdownEl = document.querySelector(".pf-lorax-pdp-offer-countdown");
    const expiryTextEl = document.querySelector(".pf-lorax-pdp-offer-expiry-text");
...
document.addEventListener("DOMContentLoaded", function () {
    const dateElement = document.querySelector(".pf-lorax-pdp-shipping-bar-sb-date");
...
document.addEventListener('DOMContentLoaded', function() {
    const socialProofLinks = document.querySelectorAll('a.pf-lorax-pdp-social-proof');
    
    socialProofLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            e.preventDefault();
            
            const targetElement = document.querySelector('div.pf-lorax-reviews');
...
document.addEventListener('DOMContentLoaded', function() {
    const accordionItems = document.querySelectorAll('.pf-lorax-accordion');
    
    if (accordionItems.length > 0) {
        accordionItems.forEach(item => {
            const summary = item.querySelector('summary');
            const content = item.querySelector('.pf-lorax-accordion__content');
...
document.addEventListener('DOMContentLoaded', function() {
    const stickyCtaElement = document.getElementById('pf-lorax-sticky-cta');
    const formSection = document.querySelector('.pf-lorax-form');
...
document.addEventListener("DOMContentLoaded", function () {
    const offerItems = document.querySelectorAll('.pf-lorax-pdp-offer-item');
    const atcButtons = document.querySelectorAll('.pf-lorax-pdp-atc');
    const priceDisplay = document.querySelector('.pf-lorax-pdp-atc-price-main');
    const strikethroughPrice = document.querySelector('.pf-lorax-pdp-atc-price-su');
    
    // Also get sticky CTA price elements
    const stickyPriceDisplay = document.querySelector('.pf-lorax-pdp-sticky .pf-lorax-pdp-atc-price-main');
    const stickyStrikethroughPrice = document.querySelector('.pf-lorax-pdp-sticky .pf-lorax-pdp-atc-price-su');
    ...
    if (offerItems.length > 0) {
        offerItems.forEach(item => {
            item.addEventListener('click', function(e) {
                ...
                sessionStorage.setItem('pfLoraxSelectedQuantity', quantity);
    ...
    atcButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            if (!sessionStorage.getItem('pfLoraxSelectedQuantity')) {
                sessionStorage.setItem('pfLoraxSelectedQuantity', '1');
            }
...
function fireViewContentEvent() {
    ...
    const selectedQuantity = sessionStorage.getItem('pfLoraxSelectedQuantity') || '1';
    const quantity = parseInt(selectedQuantity);

    // Lorax Pro product details
    const productData = {
        id: 'lorax-pro-bundle-' + quantity,
        name: `Lorax Pro Barefoot Shoes (${quantity} Pairs)`,
        category: 'footwear',
        brand: 'Peak Footwear',
        price: quantity === 1 ? 49.95 : (quantity === 2 ? 44.95 : 39.95),
        quantity: quantity,
        sku: 'LORAX-PRO-' + quantity,
        image: 'https://cdn.29next.store/media/peakfootwear/uploads/Lorax_-_Main.webp'
    };
    ...
    if (window.NextDataLayer) {
        ...
        console.log('View Content event fired for Lorax Pro bundle:', productData);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const galleryNavButtons = document.querySelectorAll('.pf-lorax-sli-carousel-button');
    galleryNavButtons.forEach(button => {
        button.setAttribute('role', 'button');
        button.setAttribute('tabindex', '0');
        
        if (button.classList.contains('pf-lorax-sli-prev')) {
