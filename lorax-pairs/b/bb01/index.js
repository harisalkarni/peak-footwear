...
function calculatePackageId(color, size, quantity) {
    // Color order and IDs
    const colors = [
        'white-pink',    // Base: 1-10
        'black',         // Extended: 11-22
        'pink',          // Base: 23-32
        'white-black',   // Extended: 33-44
        'white-gray',    // Extended: 45-56
        'blue',          // Extended: 57-68
        'orange',        // Extended: 69-80
        'white-blue'     // Extended: 81-92
    ];
    
    // Size order and IDs (NEW BACKEND FORMAT)
    const baseSizes = [
        'US Women 7 - US Men 5',
    ...
    // Extended sizes (only for certain colors)
    const extendedSizes = [
        'US Women 15 - US Men 13',
        'US Women 16 - US Men 14'
    ];
    
    // Colors that have extended sizes
    const extendedSizeColors = ['black', 'white-black', 'white-gray', 'blue', 'orange', 'white-blue'];
    
    const colorIndex = colors.indexOf(color);
    const sizeIndex = baseSizes.indexOf(size);
    const extendedSizeIndex = extendedSizes.indexOf(size);
    ...
    console.log('Package ID calculation debug:', {
        ...
        sizeIndex: sizeIndex,
        extendedSizeIndex: extendedSizeIndex
    });
    ...
    // Calculate base offset for quantity (1 pair = 0, 2 pairs = 91, 3 pairs = 182)
    const quantityOffset = (quantity - 1) * 91;
    
    // Calculate color group offset
    let colorOffset = 0;
    for (let i = 0; i < colorIndex; i++) {
        if (extendedSizeColors.includes(colors[i])) {
            colorOffset += 12; // Extended colors have 12 sizes
        } else {
            colorOffset += 10; // Standard colors have 10 sizes
        }
    }
    ...
    } else if (extendedSizeIndex !== -1 && extendedSizeColors.includes(color)) {
        sizeOffset = 9 + extendedSizeIndex; // Extended sizes start after base sizes
    } else {
        console.error('Size not found for color:', size, color);
        return null; // Invalid size for this color
    }
    
    packageId = quantityOffset + colorOffset + sizeOffset + 1;
    ...
    if (checkoutButton) {
        checkoutButton.addEventListener('click', async function(e) {
            ...
            setTimeout(() => {
                ...
                const checkoutUrl = preserveUrlParameters('/lorax-pairs/c/co01');
