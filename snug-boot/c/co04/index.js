// Olympus MV Selection - Snug Boot Configuration
// Peak Footwear - Multi-Variant Checkout (29next product 15018)
...
// 29next product this checkout sells (Snug Boot - Non-slip & waterproof winter barefoot shoe).
// Every package in the campaign that does not belong to this product is ignored below,
// so the same campaign can host Lorax Pro, upsells and Snug Boot side by side.
const TARGET_PRODUCT_ID = 15018;
...
// so a merchandising tweak in 29next does not break the mapping.
...
/**
 * Normalizes a 29next colour label to the key used in COLOR_DISPLAY_TO_SLUG.
 * e.g. "Brown (Almost sold out!)" → "brown"
 */
function normalizeColorDisplay(value) {
 ...
 * e.g. "Snug Deal - Brown (Almost sold out!) / US Women 8/8.5 - US Men 6/6.5"
 */
...
function buildPackageMapFromCampaign() {
  ...
  cd.packages.forEach(pkg => {
    // Only map packages that belong to this checkout's product — everything else in the
    // campaign (Lorax Pro, upsells, special offers) is handled by other pages.
    if (String(pkg.product_id) !== String(TARGET_PRODUCT_ID)) return;
    ...
    const colorDisplay = normalizeColorDisplay(dashIdx !== -1 ? prefix.substring(dashIdx + 3) : prefix);
...
// ============================================
// CONFIGURATION SECTION - SNUG BOOT - NEXTCOMMERCE
// ============================================
const CONFIG = {
  // Navigation URLs
  urls: {
    checkoutStep2: '/snug-boot/c/co03/', // Billing page after variant selection
  },

  // Product Images by Color (Snug Boot - Nextcommerce, product ID 15018).
  // These are the variant thumbnails 29next already serves for each colour,
  // so they stay in sync with the catalogue instead of drifting from it.
  colors: {
    images: {
      'brown': 'https://d1w87fp8j98gad.cloudfront.net/media/thumbnails/bee1aaf80c9644aaab44c2d70f8cd2e3.webp',
      'black': 'https://d1w87fp8j98gad.cloudfront.net/media/thumbnails/cceca4946adc44fca505888295613b27.webp',
      'light-gray': 'https://d1w87fp8j98gad.cloudfront.net/media/thumbnails/f69ee35c58d84943ba3c3112dde4a3cc.webp',
      'coffee': 'https://d1w87fp8j98gad.cloudfront.net/media/thumbnails/f298083627d440b1aafa5da3806b404c.webp',
      'cream': 'https://d1w87fp8j98gad.cloudfront.net/media/thumbnails/8f4a293b4eec4ee785fa3f09c67952bc.webp',
    },
    styles: {
      'brown': '#C89A6B',
      'black': '#1A1A1A',
  ...
  exitIntent: {
    enabled: false,
    image: 'https://d1w87fp8j98gad.cloudfront.net/media/thumbnails/bee1aaf80c9644aaab44c2d70f8cd2e3.webp',
...
class TierController {
  ...
  _setDefaultsWithoutCart() {
    ...
    // getAvailableVariantAttributes returns display names like "Brown (Almost sold out!)".
    // All internal maps (CAMPAIGN_PACKAGE_MAP, CONFIG.colors.images, etc.) are keyed by slug
    // like "brown", so we must convert the display name → slug before storing.
    const defaultColorDisplay = colors.find(c =>
      COLOR_DISPLAY_TO_SLUG[normalizeColorDisplay(c)] === CONFIG.defaults.color
    ) || colors[0];
    // Convert display name → slug (e.g. "Light Gray" → "light-gray")
    const defaultColorSlug = COLOR_DISPLAY_TO_SLUG[normalizeColorDisplay(defaultColorDisplay)]
  ...
  _getProductId() {
    ...
    // Use the first package that belongs to this checkout's product (Snug Boot, 15018).
    const mainPkg = campaign?.packages?.find(
      pkg => String(pkg.product_id) === String(TARGET_PRODUCT_ID)
    );
  ...
  // Update size options based on selected color (not every colour stocks every size)
